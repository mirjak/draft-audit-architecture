...
* **WI-6: Audit Context Propagation Protocol Extensions.**
  This could be realized by an HTTP headers carrying the Audit Context, e.g. a workflow-wide `Audit-Trace-ID`, an immediate-redecessor `Audit-Parent-ID`, the current `Audit-Actor`, the upstream `Audit-On-Behalf-Of`, the WI-1-1 tracing chain as `Audit-Delegation-Chain`, and a reference to the current `Audit-Auth-State`.
